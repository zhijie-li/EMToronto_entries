...
These files are generated by pickle.dump() the data files. Before this is done, these files are the images/graphs displayed during and after the job is running. These files are also saved in the mongoDB gridFS. They are referred by the mongoDB['events'] entries, which are the text displayed in the web interface of the running job.
